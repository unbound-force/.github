...
user's message for a PR number argument. If present, validate
that the complete argument matches `^[1-9][0-9]*$`. If it
does not, **STOP** with:
> "Invalid PR number. Must be a positive integer."

Only after validation succeeds, set `PR_NUMBER` to that
value. All subsequent steps use `<PR_NUMBER>` — no
auto-detection commands are needed or permitted. Never use
an unvalidated argument in a command, API route, branch name,
temporary path, or cache path.
