...
2. **Sanitize keywords**: Build a search string of at most 200 characters using only letters, digits, spaces, and the characters `._+/#:-`. Remove all other characters, collapse repeated whitespace, and discard tokens beginning with `--`. If no keywords remain, skip duplicate search. This strict allowlist prevents shell and CLI flag injection; a denylist is insufficient.
...
2. **Duplicate check**: Before creating, derive `<sanitized-child-title>` using the same allowlist, 200-character limit, whitespace normalization, leading-flag rejection, and empty-result handling defined in Phase 1.4. Then search for existing issues matching the sanitized title:
