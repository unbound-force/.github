...
**Argument parsing** (before any tool calls): Check the user's message for a PR number argument. If present, validate that the complete argument matches `^[1-9][0-9]*$`. If it does not, **STOP** with:
> "Invalid PR number. Must be a positive integer."

Only after validation succeeds, set `PR_NUMBER` to that value. All subsequent steps use `<PR_NUMBER>` — no auto-detection commands are needed or permitted. Never use an unvalidated argument in a command, API route, branch name, temporary path, or cache path.
...
```bash
# Write reply body and JSON payload to restrictive temp files.
# The API expects an object with a body property.
umask 077
REPLY_BODY_FILE=$(mktemp)
REPLY_PAYLOAD_FILE=$(mktemp)
trap 'rm -f "$REPLY_BODY_FILE" "$REPLY_PAYLOAD_FILE"' EXIT
cat > "$REPLY_BODY_FILE" << 'REPLY_EOF'
<reply content here>
REPLY_EOF
...
gh api repos/{owner}/{repo}/pulls/<PR_NUMBER>/comments/<comment_id>/replies \
  --method POST --input "$REPLY_PAYLOAD_FILE"
