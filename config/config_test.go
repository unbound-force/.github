...
var configPath = flag.String("config", "../org/config.yaml", "Path to peribolos config")
...
func TestOrgs(t *testing.T) {
	...
	for orgName, orgCfg := range cfg.Orgs {
		members := normalize(sets.New(orgCfg.Members...))
		admins := normalize(sets.New(orgCfg.Admins...))
		...
		if diff := approvers.Difference(admins); len(diff) > 0 {
			t.Errorf("users do not match in CODEOWNERS and org admins '%s': %s", orgName, strings.Join(diff.UnsortedList(), ", "))
		}

		if n := len(approvers); n < 1 {
			t.Errorf("Require at least 1 approver, found %d: %s", n, strings.Join(approvers.UnsortedList(), ", "))
		}
		...
		if orgCfg.Teams != nil {
			for _, ref := range teamRefs.UnsortedList() {
				...
				if _, exists := orgCfg.Teams[teamName]; !exists {
					t.Errorf("CODEOWNERS references team '%s' which does not exist in org '%s' teams", ref, orgName)
				}
			}
		}

		if both := admins.Intersection(members); len(both) > 0 {
			t.Errorf("users in both org admin and member roles for org '%s': %s", orgName, strings.Join(both.UnsortedList(), ", "))
		}
		...
		if !isSorted(orgCfg.Admins) {
			t.Errorf("admins for %s org are unsorted", orgName)
		}
		if !isSorted(orgCfg.Members) {
			t.Errorf("members for %s org are unsorted", orgName)
		}

		if orgCfg.Teams != nil {
			for _, err := range testTeamMembers(orgCfg.Teams, admins, allOrgMembers, orgName) {
				t.Errorf("%v", err)
			}
		}
	}
